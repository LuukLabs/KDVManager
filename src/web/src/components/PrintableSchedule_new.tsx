    ...
    >
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="h6" gutterBottom>
          {t("Van")} {formatDate(schedule.startDate || "")} {t("t/m")}{" "}
          {schedule.endDate ? formatDate(schedule.endDate) : t("Onbepaald")}
        </Typography>
      </Box>
      ...
      <Table size="small" sx={{ border: "1px solid #000" }}>
        ...
        <TableBody>
          {sortedRules.map((rule, index) => {
            const hoursPerDay =
              rule.startTime && rule.endTime ? calculateHours(rule.startTime, rule.endTime) : 0;
            const daysInPeriod = calculateDaysInPeriod(
              schedule.startDate || "",
              schedule.endDate || null,
              rule.day as number,
            ...
            return (
              <TableRow key={index}>
                ...
                <TableCell sx={{ borderRight: "1px solid #000" }}>{daysInPeriod}</TableCell>
                <TableCell sx={{ borderRight: "1px solid #000" }}>
                  {rule.groupName || "-"}
                </TableCell>
