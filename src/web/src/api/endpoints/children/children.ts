...
export const archiveChild = (id: string, signal?: AbortSignal) => {
  return executeFetch<void>({ url: `/crm/v1/children/${id}/archive`, method: "POST", signal });
};

export const getArchiveChildMutationOptions = <TError = unknown, TContext = unknown>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["archiveChild"];
  const { mutation: mutationOptions } = options
    ? options.mutation && "mutationKey" in options.mutation && options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
  ...
  const mutationFn: MutationFunction<Awaited<ReturnType<typeof archiveChild>>, { id: string }> = (
    props,
  ) => {
    const { id } = props ?? {};

    return archiveChild(id);
  };
...
export type ArchiveChildMutationError = unknown;

export const useArchiveChild = <TError = unknown, TContext = unknown>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof archiveChild>>,
      TError,
  ...
  TContext
> => {
  const mutationOptions = getArchiveChildMutationOptions(options);
